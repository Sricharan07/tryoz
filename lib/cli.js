...
const { configTargets, detectClientDetails, detectClients, removeClients, setupClients } = require("./clients");
...
async function main(argv) {
  ...
  if (parsed.command === "detect" || parsed.command === "list-agents" || parsed.command === "agents") {
    ...
    writePayload(parsed, { clients, details });
    if (!parsed.jsonOutput) printDetected(details);
    return;
  }
...
async function runSetup(options) {
  const detected = detectClientDetails();
...
function selectedClients(options, detected, opts = {}) {
  ...
  return CLIENTS.filter((client) => isSelectedByDefault(detected[client]));
}
...
async function promptClients(detected) {
  ...
  return checkbox({
    ...
    choices: CLIENTS.map((client) => ({
      ...
      checked: isSelectedByDefault(detected[client]),
      name: `${padRight(CLIENT_META[client].label, 40)} ${formatDetection(detected[client])}`
    })),
    theme: {
      style: {
...
function isSelectedByDefault(detail) {
  if (detail && typeof detail === "object") return Boolean(detail.selected);
  return Boolean(detail);
}

function formatDetection(detail) {
  if (!detail || typeof detail !== "object") return detail ? pc.green("detected") : pc.dim("not detected");
  if (detail.source === "config") return pc.yellow(detail.label);
  if (detail.source === "missing") return pc.dim(detail.label);
  return pc.green(detail.label);
}
...
function printDetected(detected) {
  for (const client of CLIENTS) {
    ...
    const status = formatDetection(detected[client]);
